...
GrafiekWeerBroedseizoen <- function(jaarkeuze, weertabel, 
...
GrafiekWeerBroedseizoenHalveMaand <- function(jaarkeuze = werk_jaar, weertabel = data_weer, 
                                              parameter = "Tgem", 
                                              opdeling_halvemaand = TRUE, 
                                                 kleur = Kleur3, 
                                                 vulkleur = Kleur3, lijndikte = 1, alpha = 0.5, puntgrootte = 3, 
                                                 NA_rm = TRUE, include_totaal = FALSE, ruimte = RuimteRondGrafiek) {
  
  weertabelsubset <- data_weer %>%
...
GrafiekWeerWinter <- function(jaarkeuze, weertabel, 
                              parameter = "Tgem", lijnkleur = Kleur3,
...
GrafiekWeerWintermaand <- function(jaarkeuze = werk_jaar, weertabel = data_weer, 
                                   parameter = "Tgem",
  ...
  weertabelsubset <- data_weer %>%
    ...
    filter(Hydrojaar <= jaarkeuze & Hydrojaar > min(data_weer$Jaar)) %>% 
    arrange(DatumTijd)

  weertabelsubset %>% 
    filter(Dag == 29 & Jaar == 2015 & Maand == 1)
  ...
  if (labels_x2) {
    ...
    labels <- c("oktober","november","december","januari","februari","maart")
  }
  ...
  if (include_totaal) {
    grafiek <- grafiek +
      geom_point(data = data_graf_bar_punt, aes(x = Periode, y = Waarde), col = kleur, size = puntgrootte) +
      scale_x_continuous(breaks = c(1,2,3,4,5,6,8), labels = c(labels,"Totaal"))
  } else {
    grafiek <- grafiek +
      scale_x_continuous(breaks = c(1,2,3,4,5,6,8), labels = c(labels))
  }
  return(grafiek)
}
