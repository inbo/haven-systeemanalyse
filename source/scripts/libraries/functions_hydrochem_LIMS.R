...
# (VAR <- Hydrochem_calc_mg_NP(VAR_DWH))
...
Hydrochem_add_Ntcalc <- function(data){
  ...
  data %>% 
    ...
    filter(any("N_Kj" %in% Component), # if N_Kj, NO3 an NO2 are present we can calculate N.T
           any("NO3" %in% Component),
           any("NO2" %in% Component)) %>% 
...
# Hydrochem_plot_titlelabs wordt in andere functies gebruikt, returns a list with title, label y en limits y
...
# Hydrochem_bar_component(data = VAR_DWH, component = "pH.25", 
...
# VAR_DWH_test %>% 
#   Hydrochem_calc_mg_NP() %>% 
#   filter(str_detect(Component, "pH "))
...
# VAR_DWH %>%
#   filter(str_starts(MeetpuntCode, "VARR|VARS")) %>%
#   Hydrochem_point_component(component = "NO3")
# (p <- VAR_DWH %>%
#   filter(str_starts(MeetpuntCode, "VARR|VARS")) %>%
